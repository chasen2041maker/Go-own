...
| [原创投资内容社区](projects/04-investment-community/README.md) | V1 完成；本地与 Linux CI 全部验证 | MySQL、JWT、分层、事务、幂等、治理、审计、集成与黑盒验收 |
...
投资社区的本地 test/vet/build/integration/acceptance 已验证；Linux CI 还从空 Compose 环境完成了镜像构建、Swagger/OpenAPI 探测与 21 接口黑盒旅程。全仓 `go vet ./...` 另有既有练习基线失败：`practice/07-functions/answers/exercise.go:70` 不可达代码，因此高级项目使用上面的项目范围 vet 作为门禁。
